...
const RightSide = ({rnd}: {rnd: number}) => {
    return (
...
const LeftSide = ({rnd}: {rnd: number}) => {
    ...
    const pics = () => mensTeesProductImage.map(imgSrc => <PictureCard key={imgSrc} imgSrc={imgSrc} rnd={rnd} />)
...
const PictureCard = ({ imgSrc, rnd }: { imgSrc: string, rnd: number }) => {
